    ...
    try {
      ...
      if (stats && stats.pending > 0) {
        alerts.push({
          id: 'pending',
          type: 'warning',
          title: 'Yêu cầu chờ mượn',
          description: `Toàn hệ thống có ${stats.pending} yêu cầu cần được phê duyệt.`,
          time: 'Ưu tiên',
      ...
      if (stats && stats.overdue > 0) {
        alerts.push({
          ...
          description: `Có ${stats.overdue} lượt mượn đã vượt quá thời hạn quy định.`,
          time: 'Khẩn cấp',
      ...
      if (alerts.length === 0) {
        alerts.push({
          ...
          description: 'Hệ thống iLibrary đang hoạt động tối ưu.',
          time: 'Hôm nay',
                ...
                <>
                  ...
                  <div className="absolute right-0 mt-3 w-96 bg-white rounded-[2rem] shadow-2xl border border-slate-100 py-6 z-40 animate-in fade-in zoom-in-95 duration-200">
                    <div className="px-8 mb-6 flex items-center justify-between">
                      <div>
                        <h4 className="text-sm font-black text-slate-900 uppercase tracking-[0.2em]">Thông báo Hệ thống</h4>
                        <p className="text-[10px] text-slate-400 font-bold uppercase mt-1 tracking-widest">Quản trị viên</p>
                      </div>
