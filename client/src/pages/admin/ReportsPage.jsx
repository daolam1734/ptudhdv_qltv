      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-2 gap-8">
            ...
            <div className="bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-sm space-y-6">
               <div className="flex items-center justify-between">
                  <h3 className="text-lg font-black text-gray-900 tracking-tight">Thống kê phí phạt</h3>
                  <CreditCard size={20} className="text-amber-500" />
               </div>
               <div className="space-y-6 py-2">
                  <div className="p-4 bg-gray-50 rounded-2xl border border-gray-100">
                     <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Tổng cộng tiền phạt</p>
                     <p className="text-xl font-black text-gray-900">{(stats?.borrowStats?.totalFines || 0).toLocaleString('vi-VN')} <span className="text-xs text-gray-400">đ</span></p>
                  </div>
                  <div className="p-4 bg-rose-50 rounded-2xl border border-rose-100">
                     <p className="text-[10px] font-black text-rose-400 uppercase tracking-widest mb-1">Số dư chưa thu</p>
                     <p className="text-xl font-black text-rose-600">{(stats?.borrowStats?.unpaidFines || 0).toLocaleString('vi-VN')} <span className="text-xs text-rose-400">đ</span></p>
                  </div>
                  <div className="flex justify-between items-center px-2">
                     <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Tỷ lệ đã thu</span>
                     <span className="text-xs font-black text-emerald-500">
                       {Math.round((((stats?.borrowStats?.totalFines || 0) - (stats?.borrowStats?.unpaidFines || 0)) / (stats?.borrowStats?.totalFines || 1)) * 100)}%
                     </span>
                  </div>
               </div>
            </div>
