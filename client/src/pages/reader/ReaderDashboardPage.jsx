...
const ReaderDashboardPage = () => {
   ...
   const handleToggleFavorite = async (bookId) => {
      ...
      try {
         ...
         if (response.success) {
            const isNowFavorite = !favorites.includes(bookId);
            setFavorites(prev =>
               isNowFavorite ? [...prev, bookId] : prev.filter(id => id !== bookId)
            );
            toast.success(isNowFavorite ? "Đã thêm vào yêu thích" : "Đã xóa khỏi yêu thích", {
               icon: isNowFavorite ? "❤️" : "🗑️",
            });
         }
      } catch (error) {
